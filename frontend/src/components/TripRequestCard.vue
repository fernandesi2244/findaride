<template>
    <h2 class="accordion-header">
        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
            ...
            <span class="status">Waiting to be accepted</span>
        </button>
    </h2>
        ...
        <div class="accordion-body">
            <div class='mb-2'>
                <div class="flex">
                    ...
                    <div class="hug-right">
                        <button class="btn btn-danger btn-sm ms-1" @click="emit('removeTripRequest', tripRequest.id)">Delete
                            trip</button>
                    </div>
                </div>
                ...
                <div v-else class="table-responsive">
                    <p class="text-start">Note: the following information represents the trips
                        <strong>before</strong> your possible addition.
                    </p>
                    <table class="table bdr">
                        ...
                        <tbody>
                            ...
                            <tr v-for="join in tripRequest.join_requests" :key="join.id"
                                :class="{ 'join-request-orange': join.status === 'pending' }">
                                <td>{{ join.trip.num_participants }}</td>
                                <td :title="join.trip.departure_location">{{ cleanLocation(join.trip.departure_location) }}</td>
                                <td :title="join.trip.arrival_location">{{ cleanLocation(join.trip.arrival_location) }}</td>
                                <td>{{ getDate(join.trip.earliest_departure_time) + " @ " +
                                    getTime(join.trip.earliest_departure_time) }}</td>
                                <td>{{ getDate(join.trip.latest_departure_time) + " @ " +
                                    getTime(join.trip.latest_departure_time) }}</td>
